...
def get_latest_updated_at_from_table(
    ...
    Returns:
        Latest updated_at timestamp in 'YYYY-MM-DD HH:MM:SS.fff' format
    """
    query = f"SELECT MAX(updated_at) FROM {schema_name}.{table_name}"
    logger.info(f"Getting latest updated_at from {schema_name}.{table_name}")
