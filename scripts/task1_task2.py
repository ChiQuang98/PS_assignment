...
def create_table(data_source_path):
    ...
    output_path = os.path.abspath(os.getenv("OUTPUT_PATH", "FINAL_TABLE.csv"))
...
def add_data_quality_expectations(suite, df):
    ...
    df["EXPECTED_REVENUE"] = df["AVG_REVENUE_PER_PURCHASE_HOURLY"] * df["TOTAL_PURCHASES_AMOUNT_HOURLY"]
    # 1. Completeness
    ...
    #3. Not null for some columns
    for col in ['APP_HOUR', 'USERID','TOTAL_SPINS_HOURLY', 'TOTAL_REVENUE_HOURLY', 'TOTAL_PURCHASES_AMOUNT_HOURLY', 
                'AVG_REVENUE_PER_PURCHASE_HOURLY', 'TOTAL_REVENUE_PER_USER_DAILY']:
        suite.add_expectation(ge.expectations.ExpectColumnValuesToNotBeNull(column=col))

    # 34. Data Type Consistency
    ...
    # 5. Reasonable Ranges
    # for this test we neede to consult with Business Analysts to get the correct range
    suite.add_expectation(ge.expectations.ExpectColumnValuesToBeBetween(column='TOTAL_SPINS_HOURLY', min_value=0, max_value=10000))
    suite.add_expectation(ge.expectations.ExpectColumnValuesToBeBetween(column='TOTAL_PURCHASES_AMOUNT_HOURLY', min_value=0, max_value=10000))


    # 6. Row count validation
    suite.add_expectation(ge.expectations.ExpectTableRowCountToEqual(value=int(df.shape[0])))

    # 7. New: Specific country values (US and CA only)
    suite.add_expectation(ge.expectations.ExpectColumnDistinctValuesToBeInSet(
        column='COUNTRY',
        value_set=['US', 'CA', 'UNKNOWN'],
    ))

    # 8. New: When purchases amount is 0, revenue should be 0
    suite.add_expectation(ge.expectations.ExpectColumnPairValuesToBeEqual(
        column_A='TOTAL_REVENUE_HOURLY',
        column_B='EXPECTED_REVENUE',
        mostly=1.0,
    ))


    # 9. Uniqueness checks
    suite.add_expectation(ge.expectations.ExpectCompoundColumnsToBeUnique(
        column_list=['USERID', 'APP_HOUR'],
        mostly=1  
    ))


    # 10. Column existence check
    expected_columns = ['APP_HOUR', 'USERID', 'COUNTRY', 'TOTAL_SPINS_HOURLY', 
                       'TOTAL_REVENUE_HOURLY', 'TOTAL_PURCHASES_AMOUNT_HOURLY',
                       'AVG_REVENUE_PER_PURCHASE_HOURLY', 'TOTAL_REVENUE_PER_USER_DAILY',
                       'EXPECTED_REVENUE']
    suite.add_expectation(ge.expectations.ExpectTableColumnsToMatchOrderedList(
...
def validate_table(df):
    ...
    gx_project_dir = os.path.abspath(os.getenv('GX_PROJECT_DIR', 'gx_project'))
